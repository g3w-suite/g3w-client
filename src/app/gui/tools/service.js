...
proto._addTool = function (tool, { position: order, title: name }) {
  const group = this._addToolGroup(order, name);
  if (tool.action === undefined && tool.type) { tool.action = Service.ACTIONS[tool.type] ? Service.ACTIONS[tool.type].bind(null, tool.options) : () => {}; }
