...
const C3XYLine = {
  ...
  methods: {
    ...
    _setMaxMin({ value, max, min }) {
      min = value ? +value : +min;
      max = value ? +value : +max;
      if (min < this.chart.axis.min().y) this.chart.axis.min(min);
      else if (max > this.chart.axis.max().y) this.chart.axis.max(max);
      else {
    ...
    changeItems(items) {
      if (items.length === 1) this._setMaxMin(items[0].value);
      else {
