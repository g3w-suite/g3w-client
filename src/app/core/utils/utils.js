...
export function truefnc() {
  return true;
}

export function falsefnc() {
  return true;
}
...
export function getValueFromG3WObjectEvent() {
  // TODO
}
 ...
 * @returns {boolean}
 */
export const isURL = (url) => url && url.match(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g);

export function sanitizeUrl({ url, reserverParameters = [] } = {}) {
...
export function imageToDataURL({ src, type = 'image/jpeg', callback = () => {} }) {
...
export function downloadCSVLayerFeatures({ layer, alias = true } = {}) {
  // get headers
...
export function downloadCSV({ filename = getUniqueDomId(), items = [] } = {}) {
...
export function getTimeoutPromise({ timeout = TIMEOUT, resolve, data }) {
...
export function createSingleFieldParameter({field, value, operator = 'eq', logicop = null}) {
...
export function createFilterFromString({ layer, search_endpoint = 'ows', filter = '' }) {
 ...
 * @returns {*}
 */
export function createFilterFormInputs({ layer, search_endpoint = 'ows', inputs = [] }) {
  ...
  switch (search_endpoint) {
    ...
    case 'api':
      ...
      const fields = inputs.map((input, index) => createSingleFieldParameter({
 ...
 */
export function createFilterFormField({layer, search_endpoint = 'ows', field, value, operator = 'eq'}) {
  let filter;
  switch (search_endpoint) {
    case 'ows':
      const expression = new Expression();
...
export const trimValue = value => value.replace(/ /g, '')
