...
import {
  Style, Circle, Stroke, Fill, Icon, Text,
} from 'ol/style';
import { Feature, Collection } from 'ol';
import { Vector as VectorLayer } from 'ol/layer';
import { Vector as VectorSource } from 'ol/source';
import {
  GPX, GeoJSON, WMSGetFeatureInfo, KML, WKT,
} from 'ol/format';
...
export function createOlLayer(options = {}) {
  const { id } = options;
  const { features } = options;
  const { geometryType } = options;
  const { color } = options;
  let { style } = options;
...
export function createWMSLayer({url, name, projection, layers = [],} = {}) {
...
export function createVectorLayerFromGeometry(geometry) {
  const feature = new Feature(geometry);
  return createVectorLayerFromFeatures(feature);
}
...
export function createStyleFunctionToVectorLayer(options = {}) {
  const styleFunction = (feature, resolution) => {
    ...
    const style = getDefaultLayerStyle(geometryType, { color });
...
export function createSelectedStyle({ geometryType, color = 'rgb(255,255,0)', fill = true } = {}) {
  let style = null;
 ...
 */
export function getQueryLayersPromisesByBBOX(layers, {
  bbox, filterConfig = {}, feature_count = 10, multilayers = false,
}) {
  let queriesPromise;
  const geometry = fromExtent(bbox);
...
export function getQueryLayersPromisesByGeometry(layers, {
  multilayers = false, geometry, filterConfig = {}, projection, feature_count = 10,
} = {}) {
  const d = $.Deferred();
  const queryResponses = [];
  const queryErrors = [];
  ...
  } else if (layers.length === 0) d.resolve([]);
  else {
    let layersLenght = layers.length;
    layers.forEach((layer) => {
      const layerCrs = layer.getProjection().getCode();
...
export function getQueryLayersPromisesByCoordinates(layers, {
  coordinates, feature_count = 10, query_point_tolerance = QUERY_POINT_TOLERANCE, multilayers = false, reproject = true,
} = {}) {
  const d = $.Deferred();
...
export function transformBBOX({ bbox, sourceCrs, destinationCrs } = {}) {
...
export function getFeaturesFromResponseVectorApi(response = {}, { type = 'vector' } = {}) {
  if (response.result) {
    ...
    switch (type) {
      case 'result':
        return covertVectorFeaturesToResultFeatures(features);
        break;
      case 'vector':
      default:
        return features;
...
export function covertVectorFeaturesToResultFeatures(features = []) {
...
export function splitFeatures({ features = [], splitfeature } = {}) {
  ...
  features.forEach((feature) => {
    const geometries = splitFeature({ feature, splitfeature });
...
export function splitFeature({ feature, splitfeature } = {}) {
  const geometries = {
  ...
  switch (splitType) {
    case 'LineString':
      ...
      if (featureGeometryType.indexOf('Polygon') !== -1) {
        ...
        } else return splitGeometryLine(geometries.split, geometries.feature);
      }
      break;
  }
...
export function isMultiGeometry(geometry) {
  return !Geometry.isMultiGeometry(geometry.getType());
}
...
export function convertSingleMultiGeometry(geometry, toGeometryType) {
  if (toGeometryType) {
    const isFromGeometryMulti = isMultiGeometry(geometry);
    const isToGeometryMulti = Geometry.isMultiGeometry(toGeometryType);
    if (isFromGeometryMulti && !isToGeometryMulti) return multiGeometryToSingleGeometries(geometry);
    if (!isFromGeometryMulti && isToGeometryMulti) return singleGeometriesToMultiGeometry(geometry);
  }
  return geometry;
}

export function dissolve({ features = [], index = 0, clone = false } = {}) {
  const parser = new jsts.io.OL3Parser();
...
export function crsToCrsObject(crs) {
  ...
  if (toRawType(crs) === 'Object' && crs.epsg) crs.epsg = normalizeEpsg(crs.epsg);
  else {
    crs = {
 ...
 * @constructor
 */
export function ConvertDEGToDM({ deg, output = 'Array' } = {}) {
 ...
 * @constructor
 */
export function ConvertDMSToDEG({ dms, type = 'Array' }) {
  const dms_Array = type === 'Array' ? dms : dms.split(/[^\d\w\.]+/);
  const degrees = 1 * dms_Array[0];
  const minutes = 1 * dms_Array[1];
  ...
  if (direction == 'S' || direction == 'W') deg *= -1;
  return 1 * deg;
}
...
export function parseAttributes(layerAttributes, featureAttributes) {
  const featureAttributesNames = Object.keys(featureAttributes).filter((featureAttributesName) => geometryFields.indexOf(featureAttributesName) === -1);
...
const geoutils = {
  ...
  covertVectorFeaturesToResultFeatures,
  splitGeometryLine,
  splitFeatures,
  splitFeature,
