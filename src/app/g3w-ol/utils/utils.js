...
import {
  Polygon, MultiPolygon, MultiLineString, LineString,
} from 'ol/geom';
...
export function getAreaMessageText({
  unit, geometry, projection, segments = [],
}) {
  const useSphereMethods = needUseSphereMethods(projection);
...
export function formatMeasure({ geometry, projection } = {}, options = {}) {
  ...
  } if (Geometry.isPolygonGeometryType(geometryType)) {
    const segments = geometry.getLinearRing().getCoordinates();
    return getAreaMessageText({
      unit, geometry, projection, segments,
    });
  }
}
...
export function transformMeterArea(area, tounit) {
  switch (tounit) {
    case 'nautical':
      area *= 0.000000291553349598122862913947445759414840765222583489217190918463024037990567;
      break;
  }
  return area;
}
