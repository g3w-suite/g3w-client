...
export default new (class GUI extends Emitter {
  ...
  async _layout(param) {
    ...
    const viewW           = document.getElementById('app').getBoundingClientRect().width - document.querySelector(".main-sidebar").getBoundingClientRect().width - document.querySelector('.main-sidebar').getBoundingClientRect().left - window.scrollX;
    const viewH           = window.innerHeight - document.querySelector('.navbar').offsetHeight;
  ...
  */
  ...
  async getLegendSrc({ change = false, all = false } = {}) {
    ...
    const TREE = Object
      .values(ApplicationState.layers)
      .flatMap(s => s.showOnCatalog() ? ({ tree: s.getLayersTree(), storeid: s.getId() }) : []);

    for (const t of TREE) {
      let layers = _traverse(t.tree);
      for (const tree of t.tree) {
        try {
          if (change && (0 === tree?.legendurls?.length || layers.some(l => l.legend.change) || ApplicationState.project.state.context_base_legend)) {
            layers.filter(l => l.legend.change).forEach(l => l.legend.change = false);
          }

          const URLS    = [];                             // reset layers url
          const _layers = layers.filter(l => l.geolayer); // filter geolayer
          const HTTP    = { GET: {}, POST: {} };
