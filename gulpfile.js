...
function setNODE_ENV() {
  ...
  console.log(`[G3W-CLIENT] loaded plugins: {\n  ${local_plugins.map(pluginName => GREEN__ + pluginName + __RESET + ': '+ get_version(pluginName)).join('\n  ')}\n}\n`);
}
 ...
 * @since 3.10.0
 */
function get_version(pluginName, src) {
  src = src || (pluginName ? `${g3w.pluginsFolder}/${pluginName}` : '.');
 ...
 * @since 3.10.0
 */
async function set_version(pluginName) {
  const src     = pluginName ? `${g3w.pluginsFolder}/${pluginName}` : '.';
  const version = get_version(pluginName, src);
  fs.writeFileSync(`${src}/${ pluginName ? 'config' : 'src' }/version.js`, `/* Generated by ${pluginName ? 'g3w-client@v' + packageJSON.version : 'gulpfile.js'}, do not edit manually */\n\nexport default '${version}';`);
 ...
 * @since 3.10.0
 */
async function browserify_plugin(pluginName, watch = true, ctx = false) {
  const is_client = 'client' === pluginName;

  let src          = `${g3w.pluginsFolder}/${pluginName}`;                // plugin folder (git source)
  ...
  const path = is_client ? '.' : `${g3w.pluginsFolder}/${pluginName}`;
  const _src = is_client ? `./src/index.${production ? 'prod' : 'dev'}.js` : `./${pluginName}/index.js`;

  const opts = {
    basedir: is_client ? './'                                            : `${g3w.pluginsFolder}`,
    paths:   is_client ? ['./src/', './src/app/']                        : [`${g3w.pluginsFolder}`],
    debug: !production,
    cache: {},
    packageCache: {},
    insertGlobals: true,
    insertGlobalVars: {
      // inject plugin version (commit + branch). In case of new release (tag) no branch is added to version
      __git__: () => {
        try {
          let branch = execSync(`git -C  ${path} rev-parse --abbrev-ref HEAD`, { encoding: 'utf8' }).trim();
          if (branch && 'HEAD' !== branch.trim()) {
            return JSON.stringify({ branch, commit: execSync(`git -C  ${path} log -n 1 --pretty=format:"%H"`, { encoding: 'utf8' }).trim().substring(0,7) });
          }
        } catch(err) {
    ...
    ignore: (is_client && production ? ['./src/index.dev.js' ] : undefined), // ignore dev index file (just to be safe)
    plugin: [
    ...
    transform: [
      ...
      (is_client ? imgurify : undefined)
    ].filter(Boolean),
  };

  if (is_client) {
    console.log('\n' + INFO__ + 'App entry point:' + __RESET + ' → ' + _src + '\n');
  }

  const bundler = browserify(_src, opts)
    .external(is_client ? dependencies : [])
  ...
  const rebundle = async () => {
    ...
    await set_version(is_client ? undefined : pluginName);
    ...
    return bundler
      ...
      .pipe(source(`${ is_client ? '' : (src + '/') }build.js`))
      .pipe(buffer())
      .pipe(gulpif(production, sourcemaps.init()))
      .pipe(gulpif(production, uglify({ compress: { drop_console: true } }).on('error', gutil.log)))
      .pipe(rename(is_client ? 'app.min.js' : 'plugin.js'))
      .pipe(gulpif(production, sourcemaps.write(is_client ? '.' : src)))
      .pipe(gulpif(!is_client, concat('plugin.js')))
      .pipe(gulpif(!is_client, gulp.dest(src)))                         // put plugin.js to plugin folder (git source)
...
gulp.task('browserify:app', async function() {
  ...
  return await browserify_plugin('client', true, this);
});
...
gulp.task('version', function() {
  set_version();                                                                // client 
  local_plugins.forEach(pluginName => set_version(pluginName)); // plugins
});
