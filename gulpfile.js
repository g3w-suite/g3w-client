...
function setNODE_ENV() {
  ...
  console.log(`[G3W-CLIENT] loaded plugins: {\n  ${Object.entries(loaded_plugins).map(([pluginName, version]) => (GREEN__ + pluginName + __RESET + ': '+ version)).join('\n  ')}\n}\n`);
}
...
const loaded_plugins = Object.fromEntries(
  fs
  .readdirSync(g3w.pluginsFolder)
  .filter(file => file !== 'client' && file.indexOf('_templates') === -1 && fs.statSync(`${g3w.pluginsFolder}/${file}`).isDirectory() && fs.statSync(`${g3w.pluginsFolder}/${file}/plugin.js`).isFile())
  .map(pluginName => {
    let version;
    try {
...
const get_git_info = (path='.') => {
  try {
    ...
    let branch = execSync(`git -C  ${path} rev-parse --abbrev-ref HEAD`, { encoding: 'utf8' }).trim();
    // in case of branch, get commit and add it to version
    if (branch && 'HEAD' !== branch.trim()) {
      return {
        branch,
        commit: execSync(`git -C  ${path} log -n 1 --pretty=format:"%H"`, { encoding: 'utf8' }).trim().substring(0,7)
      };
    }
  } catch(err) {
...
const set_version = async (pluginName) => {

  const src    = pluginName ? `${g3w.pluginsFolder}/${pluginName}` : '.';
  const config = pluginName ? `/config` : '/src';
  ...
  try {
    ...
    console.warn(YELLOW__ + '[WARN] ' + __RESET + 'package.json not found (' + GREEN__ + src + __RESET + ')' );
  }

  const _header  = pluginName ? `# g3w-client-plugin-${pluginName} v${version}` : `# G3W-CLIENT v${version}`;

  fs.writeFileSync(`${src}${config}/version.js`, `/* Generated by ${pluginName ? 'g3w-client@v' + packageJSON.version : 'gulpfile.js'}, do not edit manually */\n\nexport default '${version}';`);
  fs.readFile(`${src}/README.md`, 'utf8', function (_, data) {
    data = (data || '').toString().split("\n");
    data.splice(0, 1, _header);
...
const browserify_plugin = async (pluginName, watch = true) => {
  ...
  const bundler = browserify(`./${pluginName}/index.js`, {
    ...
    insertGlobals: true,
    insertGlobalVars: {
      __git__: () => JSON.stringify(get_git_info(`${g3w.pluginsFolder}/${pluginName}`))
    },
    plugin: [
    ...
    transform: [
      vueify,
      [ babelify, { babelrc: true } ], // looks for .babelrc in the same directory as the input file, then in parent directory, ... (interpreted as JSON and used as Babel options)
      [ stringify, { appliesTo: { includeExtensions: ['.html'] } } ],
    ],
  })
  .on('update', ([file])  => watch && !production && -1 === file.indexOf('version.js') && rebundle()) // no need to rebundle "version.js" changes
  ...
  const rebundle = async () => {
    ...
    /**@TODO Maybe need to check when need to change version or not (ex. package.json version change) */
    await set_version(pluginName);
...
gulp.task('browserify:app', async function() {
  ...
  const bundler = browserify(src, {
      ...
      insertGlobals: true,
      insertGlobalVars: {
        __git__: () => JSON.stringify(get_git_info())
      },
      plugin: [
    ...
    .on('update', async ([file])  => !production && -1 === file.indexOf('version.js') && rebundle()) // no need to rebundle "version.js" changes
...
gulp.task('version', function() {
  set_version();                                                                              // client 
  Object.entries(loaded_plugins).forEach(([pluginName, version]) => set_version(pluginName)); // plugins
});
