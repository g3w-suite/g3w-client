...
/**
 * Set up g3w-client app version
 * @return {Promise<void>}
 */
const set_app_version = async () => {

  const version = await create_version();

  console.log(`[G3W-CLIENT] -> VERSION: ${version}`)
 ...
 * @return {Promise<void>}
 */
const set_plugin_version = async (pluginName) => {

  const version = await create_version(`${g3w.pluginsFolder}/${pluginName}`);

  fs.writeFileSync(`${g3w.pluginsFolder}/${pluginName}/config/version.js`, `/* WARNING: this file is autogenerated by gulpfile.js, please do not edit manually */\n\nexport default '${version}';`);
  fs.readFile(`${g3w.pluginsFolder}/${pluginName}/README.md`, 'utf8', function (_, data) {
    data = (data || '').toString().split("\n");
...
const browserify_plugin = async (pluginName, watch = true) => {
  ...
  const rebundle = async () => {
    ...
    await set_plugin_version(pluginName);
...
gulp.task('version', function() {
  ...
  Object.entries(loaded_plugins).forEach(([pluginName, version]) => {
    ...
    fs.writeFileSync(`${src}/_version.js`, `/* WARNING: this file is autogenerated by gulpfile.js (g3w-client@v${packageJSON.version}), do not edit manually */\n\nexport default '${version}';`);
